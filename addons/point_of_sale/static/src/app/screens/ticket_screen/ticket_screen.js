...
export class TicketScreen extends Component {
  ...
  onClickOrder(clickedOrder) {
    ...
    if (
      (!clickedOrder || clickedOrder.locked) &&
      ...
      if (firstLine) {
        this._state.ui.selectedOrderlineIds[clickedOrder.backendId] =
          firstLine.id;
      }
    }
  }
  ...
  async onDoRefund() {
    ...
    for (const refundDetail of allToRefundDetails) {
      ...
      const destinationOrderline = originalToDestinationLineMap.get(
        originalOrderline.id
      );
      ...
      if (
        originalOrderline.comboLines &&
        originalOrderline.comboLines.length > 0
      ) {
  ...
  getFilteredOrderList() {
    ...
    const filterCheck = (order) => {
      if (this._state.ui.filter && this._state.ui.filter !== "ACTIVE_ORDERS") {
        const screen = order.get_screen_data();
        return (
          this._state.ui.filter === this._getScreenToStatusMap()[screen.name]
        );
      }
      return true;
    };
    ...
    const searchCheck = (order) => {
      ...
      return (
        repr && repr.toString().toLowerCase().includes(searchTerm.toLowerCase())
      );
    };
  ...
  getStatus(order) {
    if (order.locked) {
      ...
      const screen = order.get_screen_data();
      return this._getOrderStates().get(
        this._getScreenToStatusMap()[screen.name]
      ).text;
    }
  }
  /**
  ...
  _doesOrderHaveSoleItem(order) {
    ...
    const refundableQty =
      theOrderline.get_quantity() - theOrderline.refunded_qty;
  ...
  _prepareAutoRefundOnOrder(order) {
    ...
    if (
      this.pos.isProductQtyZero(refundableQty - 1) &&
      toRefundDetail.qty === 0
    ) {
