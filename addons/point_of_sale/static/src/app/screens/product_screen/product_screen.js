...
export class ProductScreen extends ControlButtonsMixin(Component) {
  ...
  get items() {
    return (
      this.currentOrder.orderlines?.reduce(
        (items, line) => items + line.quantity,
        0
      ) ?? 0
    );
  }
  async updateSelectedOrderline({ buffer, key }) {
    ...
    if (
      ...
      this.numberBuffer.reset();
      const { confirmed, payload: inputNumber } = await this.popup.add(
        NumberPopup,
        {
          startingValue: 10,
  ...
  async _barcodeGS1Action(parsed_results) {
    const { product, lotBarcode, customProductOptions } =
      await this._parseElementsFromGS1(parsed_results);
    if (!product) {
      const productBarcode = parsed_results.find(
        (element) => element.type === "product"
      );
      return this.popup.add(ErrorBarcodePopup, {
