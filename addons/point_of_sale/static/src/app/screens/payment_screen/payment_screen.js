...
} from "@point_of_sale/app/utils/hooks";
...
export class PaymentScreen extends Component {
  ...
  setup() {
    ...
    this.payment_methods_from_config = this.pos.payment_methods.filter(
      (method) => this.pos.config.payment_method_ids.includes(method.id)
    );
  ...
  updateSelectedPaymentline(amount = false) {
    ...
    const payment_terminal =
      this.selectedPaymentLine.payment_method.payment_terminal;
  ...
  async toggleShippingDatePicker() {
    if (!this.currentOrder.getShippingDate()) {
      const { confirmed, payload: shippingDate } = await this.popup.add(
        DatePickerPopup,
        {
  ...
  async sendPaymentReverse(line) {
    ...
    const isReversalSuccessful = await payment_terminal.send_payment_reversal(
      line.cid
    );
